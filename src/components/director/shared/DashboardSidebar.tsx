...
export default function DashboardSidebar({
  ...
  return (
    <>
      {/* Arrière-plan sombre/cliquable (Overlay) pour mobile : ferme la sidebar si on clique à côté */}
      {isMobileMenuOpen && (
        <div 
      ...
      <aside
        // On active le survol uniquement sur desktop pour éviter les conflits tactiles sur mobile
        onMouseEnter={() => window.innerWidth >= 768 && setSidebarHovered(true)}
        onMouseLeave={() => window.innerWidth >= 768 && setSidebarHovered(false)}
        className={`fixed left-0 top-0 h-full bg-black md:bg-black/40 backdrop-blur-xl transition-all duration-300 ease-in-out flex flex-col ${
          isMobileMenuOpen 
            ? "translate-x-0 w-[240px] z-[60] pointer-events-auto" 
          ...
          <Link href="/" className="flex items-center gap-3">
            ...
            <div
              className={`overflow-hidden transition-all duration-300 ${
                isSidebarExpanded || isMobileMenuOpen ? "opacity-100 w-auto" : "opacity-0 w-0"
              }`}
            >
              <p
          ...
          <Link href="/login" passHref legacyBehavior>
            <button
              ...
              className={`mt-3 w-full flex items-center gap-2 px-3 py-2 text-white/80 hover:text-white rounded-lg hover:bg-white/5 transition-all ${
                isSidebarExpanded || isMobileMenuOpen ? "" : "justify-center"
              }`}
            >
