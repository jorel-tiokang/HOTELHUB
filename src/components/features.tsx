...
{/* export function Feature2() {
  ...
  <div className="bg-white rounded-2xl shadow-sm border-t-4 border-[#008080] p-8 flex flex-col h-full animateCardBoxHover">
    ...
    <ul className="space-y-4 mb-8 flex-grow">
      {["Recherche intélligente", "Réservation & annulation", "Historique de réservations", "Dépôt d'avis après séjour"].map((item, i) => (
        <li key={i} className="flex items-start text-gray-600 text-sm">
          <span className="text-[#008080] mr-3 mt-1.5 w-1.5 h-1.5 rounded-full bg-[#008080] shrink-0" />
          {item}
        </li>
      ))}
    </ul>
  ...
  <div className="bg-white rounded-2xl shadow-sm border-t-4 border-[#3C3489] p-8 flex flex-col h-full animateCardBoxHover">
    ...
    <ul className="space-y-4 mb-8 flex-grow">
      {["Gestion des hôtels", "Tableau de bord des réservations", "Consultations des avis", "Statistiques de performance"].map((item, i) => (
        <li key={i} className="flex items-start text-gray-600 text-sm">
          <span className="text-[#3C3489] mr-3 mt-1.5 w-1.5 h-1.5 rounded-full bg-[#3C3489] shrink-0" />
          {item}
        </li>
      ))}
    </ul>
  ...
  <div className="bg-white rounded-2xl shadow-sm border-t-4 border-[#CC7A5C] p-8 flex flex-col h-full animateCardBoxHover">
